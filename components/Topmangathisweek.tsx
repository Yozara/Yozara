      ...
      <section className="mb-14 px-4 sm:px-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-2xl font-extrabold text-white flex items-center gap-2">
            <TrendingUp size={22} className="text-purple-400" /> Top 10 Manga This Week
          </h2>
        </div>
          ...
          return (
            <Link href={`/manga/${item.id}`} key={item.id}>
              <motion.div
                ...
                className="relative rounded-2xl overflow-hidden bg-[#111827] border border-white/10 hover:border-purple-500/50 transition-colors cursor-pointer h-[200px] group"
              >
                ...
                <div className="absolute inset-0 flex items-center gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      ...
                      {item.seasonYear && <span className="text-white/50 text-xs">{item.seasonYear}</span>}
                      {item.chapters && <span className="text-white/50 text-xs">{item.chapters} ch</span>}
                    </div>
                    {desc && <p className="text-white/50 text-xs line-clamp-3 leading-relaxed">{desc}</p>}
                    {item.genres && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {item.genres.slice(0, 2).map(g => (
                          <span key={g} className="text-[10px] px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30">{g}</span>
                        ))}
                      </div>
                    )}
                  </div>
          ...
          return (
            <Link href={`/manga/${item.id}`} key={item.id}>
              <motion.div
                ...
                transition={{ type: "spring", stiffness: 300, damping: 22 }}
                className="relative rounded-xl overflow-hidden border border-white/10 hover:border-purple-500/50 transition-colors cursor-pointer group"
              >
